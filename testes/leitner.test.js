...
describe("Sistema de Repetição Espaçada (Leitner)", () => {
  it("deve avançar o cartão para a próxima caixa quando o usuário acertar", () => {
    ...
    assert.equal(
      cartaoAtualizado.due,
      dataInicial + 1 * MILISSEGUNDOS_POR_DIA,
      "O reagendamento da caixa 2 deve ser para +1 dia"
    );
  });
  ...
  it("deve limitar a caixa máxima ao valor 5", () => {
    ...
    assert.equal(
      cartaoAtualizado.due,
      dataInicial + 14 * MILISSEGUNDOS_POR_DIA,
      "O reagendamento da caixa 5 deve ser para +14 dias"
    );
  });
  ...
  it("deve retornar o cartão para a Caixa 1 e incrementar lapsos quando o usuário errar", () => {
    ...
    assert.equal(
      cartaoAtualizado.due,
      dataInicial + 8000,
      "Na caixa 1 o cartão deve ser reagendado para +8 segundos"
    );
  });
  ...
  it("deve adicionar um novo cartão ao baralho Leitner ao errar uma questão", () => {
    ...
    assert.equal(
      novoBaralho["br-f0-q01"].correta,
      "Amazon S3",
      "A resposta correta deve ser extraída das opções"
    );
